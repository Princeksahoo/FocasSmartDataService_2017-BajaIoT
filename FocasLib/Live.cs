...
namespace FocasLibrary
{
    ...
    {      
        ...
        public static int ReadFeedRate(ushort handle)
        {
            FocasLibBase.ODBACT aa = new FocasLibBase.ODBACT();
            var ret = FocasLibrary.FocasLib.cnc_actf(handle, aa);
            return aa.data;
        }
        ...
        /// <returns>RPM</returns>
        public static double ReadFeedRateDecimal(ushort handle)
        {
            FocasLibBase.ODBSPEED b = new FocasLibBase.ODBSPEED();
            FocasLib.cnc_rdspeed(handle, 0 , b);
            return  (((double)b.actf.data) / Math.Pow(10.0, (double)b.actf.dec));
        ...
        public static double ReadFeedRateDecimal(ushort handle, out int ret)
        {
            FocasLibBase.ODBSPEED b = new FocasLibBase.ODBSPEED();
            ret = FocasLib.cnc_rdspeed(handle, 0, b);
            //var feedRateUnit = b.actf.unit.ToString();
            return (((double)b.actf.data) / Math.Pow(10.0, (double)b.actf.dec));
        ...
        public static DateTime ReadCNCTimeStamp(ushort handle,out int ret)
        {
            ret = 0;
            DateTime cncDateTime = DateTime.MinValue;
            int date = (int)ReadMacroDouble(handle, 3011);
            ...
            string strDate = Utility.get_actual_date(date);
            string strTime = Utility.get_actual_time(time);
            if(!DateTime.TryParse(strDate + " " + strTime, out cncDateTime))
            {
                ret = -1;
            }
            return cncDateTime;
